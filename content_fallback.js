...
const isExcludedDomain = () => {
  ...
  // 사용자 지정 제외 도메인 확인
  for (const domain of userExcludedDomains) {
    if (currentHost === domain || currentHost.endsWith('.' + domain)) {
      return true;
    }
  }
...
// 초기화 시 저장된 상태 불러오기
chrome.storage.local.get({ isEnabled: true, excludedDomains: [] }, (result) => {
  isExtensionEnabled = result.isEnabled;
  userExcludedDomains = result.excludedDomains || [];
  WebPLogger.logInfo(`[WebP Extension] 확장 프로그램 상태 로드: ${isExtensionEnabled ? '활성화' : '비활성화'}`);
  WebPLogger.logInfo(`[WebP Extension] 사용자 지정 제외 도메인: ${userExcludedDomains.length}개`);
...
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
  ...
  // 제외 도메인 목록 업데이트 처리
  if (message.action === 'updateExcludedDomains') {
    userExcludedDomains = message.excludedDomains || [];
    WebPLogger.logInfo(`[WebP Extension] 사용자 지정 제외 도메인 업데이트: ${userExcludedDomains.length}개`);
    
    // 현재 페이지가 새로운 제외 목록에 포함되었는지 확인
    const isCurrentlyExcluded = isExcludedDomain();
    
    // 제외된 페이지라면 처리 중지하고 빈 함수 재정의
    if (isCurrentlyExcluded) {
      WebPLogger.logInfo('[WebP Extension] 제외 도메인으로 인해 스크립트 비활성화됨:', window.location.hostname);
      disableAllFunctions();
    } else if (isExtensionEnabled) {
      // 제외 목록에서 제거되었고 확장 프로그램이 활성화 상태라면 이미지 처리
      processAllImages();
    }
