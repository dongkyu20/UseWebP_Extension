...
function logFailedImage(cdnUrl, originalUrl, status) {
  ...
  let originalUrlDetails = { domain: '', original_path_query: '', original_filename: '', filename_base: '' };
  ...
  const logEntry = {
    ...
    message: message,
    // 원본 웹페이지 URL 정보 사용
    origin_url: originalUrlDetails.original_path_query || cdnUrlDetails.original_path_query, 
...
function parseUrlDetails(urlString) {
  try {
    ...
    return {
      ...
      original_path_query: pathname + url.search,
    };
  } catch (e) {
